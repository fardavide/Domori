...
final class CloudKitSharingManager {
    ...
    func createShare(for workspace: SharedWorkspace, context: ModelContext) async throws -> CKShare {
        ...
        workspaceRecord["name"] = workspace.name as CKRecordValue
        workspaceRecord["ownerEmail"] = workspace.ownerEmail as CKRecordValue
        workspaceRecord["createdDate"] = workspace.createdDate as CKRecordValue
        workspaceRecord["isActive"] = workspace.isActive as CKRecordValue
        
        // Create the share
        let share = CKShare(rootRecord: workspaceRecord)
        share[CKShare.SystemFieldKey.title] = "\(workspace.ownerEmail)'s Properties" as CKRecordValue
