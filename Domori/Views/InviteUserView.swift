      ...
      Form {
        ...
        Section("Invitation Preview") {
          VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
              Text("Subject: Collaboration Invitation for Workspace")
                .font(.subheadline)
                .fontWeight(.medium)
              ...
              VStack(alignment: .leading, spacing: 4) {
                if let currentUser = userManager.currentUser {
                  Text("Hi there,")
                  Text("\(currentUser.name) (\(currentUser.email)) has invited you to collaborate on its workspace in Domori.")
...
#Preview {
  ...
  InviteUserView(workspace: sampleWorkspace)
